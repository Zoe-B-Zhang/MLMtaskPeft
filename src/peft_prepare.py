...
if __name__ == "__main__":
    token_file = "data/add_tokens.json"
    data_file = "data/ft_data.json"
    data_json_file = "./data/ft_data_json.json"
    if len(sys.argv) > 1:
        print("your command here is :", sys.argv[1], "\n")
        if sys.argv[1] == "1":
            token_list, data_list = get_additional_tokens(
                "data/3GPP_21905-h10-abbr-invoc.csv"
            )
            save_list_to_json(token_list, token_file, data_list, data_file)

        elif sys.argv[1] == "2":
            token_list, data_list = read_list_from_json(token_file, data_file)
            print(token_list)
            print(data_list)
        elif sys.argv[1] == "3":
            # Load the full datasest (no predefined splits)
            # Read the JSON Lines file into a Python list of dictionaries
            data = []
            with open(data_json_file, "r") as f:
                for line in f:
                    data.append(json.loads(line))
            ...
            # Convert the list of dictionaries to a dictionary of lists (columns)
            data_columns = {key: [dic[key] for dic in data] for key in data[0]}
            # Create the Hugging Face Dataset from the dictionary of columns
            full_dataset = Dataset.from_dict(data_columns)
            print(full_dataset)
            next(iter(full_dataset))
        elif sys.argv[1] == "4":  # load_dataset
            data_json_file = "/workspace/data/peft/3gppterms_ft_w_masks_ft.json"
            model_dir = "/workspace/model/google/gemma-7b"
            print(f"test{data_json_file}")
            ...
            batch = data_collator(
                [tokenized_traindata[i] for i in range(8)]
            )  # Adjust the batch size as needed

            # Check the shape and data type of the input tensors
            print("Input shape:", {k: v.shape for k, v in batch.items()})
            print("Input data types:", {k: v.dtype for k, v in batch.items()})

            # Get the expected input shape and data type from the model
            model = AutoModelForCausalLM.from_pretrained(model_dir)
            expected_shape = model.forward.__code__.co_varnames[
                : model.forward.__code__.co_argcount
            ]
            expected_dtype = next(model.parameters()).dtype

            print("Expected input shape:", expected_shape)
            print("Expected input data type:", expected_dtype)

            print(tokenized_traindata)
            print(tokenized_eval)
            print(my_tokenizer)

            # Compare the actual and expected input shape and data type
            for k, v in batch.items():
                print(f"Shape of {k}: {v.shape}")
                print(v)
                assert (
                    v.shape == expected_shape
                ), f"Input shape mismatch for {k}: Expected {expected_shape}, got {v.shape}"
                assert (
                    v.dtype == expected_dtype
                ), f"Input data type mismatch for {k}: Expected {expected_dtype}, got {v.dtype}"

            # If no assertion errors occur, the input tensors are compatible with the model
            print("Input tensors are compatible with the model.")

        elif sys.argv[1] == "5":  #
            data_json_file = "./data/sample_wo_mask.json"  # data without mask
            print("test orig dataset")
            ...
            data_loader = DataLoader(
                tokenized_traindata, batch_size=10, collate_fn=my_collate_fn
            )
